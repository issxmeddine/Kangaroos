...
class AdaptivePMMH(mcmc.PMMH):
    ...
    def clusteringMixture(self, p, comp_count=1):
        ...
        mean_c = [np.zeros((d, 1)) for i in range(comp_count)]  # cluster means
        var_c = [np.zeros((d, d)) for i in range(comp_count)]  # cluster variances
        ...
        BIC = -2 * loglikelihood + (comp_count * (d * (d + 1) / 2 + d + 1)) * np.log(comp_count)
