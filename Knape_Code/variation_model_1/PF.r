...
particleFilterLL = function(y, p, N) {
    ...
    for (t in 2:n) {
        ...
        xx = x[ind[, t - 1], t - 1]
        delta = (y[3, t] - y[3, t - 1])
        count = floor(delta * step)
        dt = 1/step
        for (kk in 1:count){
          xx = xx + (r+s^2/2-b*exp(xx))*dt + s * sqrt(dt) * rnorm(N, 0, 1)
        }
        x[, t] = xx
...
prior = function(p) {
    ...
    bmax = 10 # s~U(0,bmax)
    return(log((abs(p[1]) < rmax)/(2 * rmax)) + log((p[2] > 0 & p[2] < smax)/smax)
           + log((p[3] > 0 & p[3] < taumax)/taumax)+ log((p[4] > 0 & p[4] < bmax)/bmax))
}
